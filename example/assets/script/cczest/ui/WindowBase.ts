...
import { CCGameWorld } from "../app/CCGameWorld";
...
export class WindowBase<T extends Component> extends Mediator implements IWindowBase {
    ...
    private _winModel: cc_zest_win_model;      //窗口模型
    ...
    public get winModel(): cc_zest_win_model { return this._winModel; }
    ...
    public load(accessId: string, isOpen: boolean, onProgress: (progress: number) => void, onComplete: () => void) {
        if (CCGameWorld.getInstance().canOpenWindow(accessId)) {
            this.loadView(isOpen, onProgress, onComplete, false);
        }
    }

    public open(onComplete: Function, ...args: any[]) {
        if (CCGameWorld.getInstance().canOpenWindow(this.accessId)) {
    ...
    /** 设置UI类型*/
     public iniViewType(): void {
        ...
        if (Assert.handle(Assert.Type.InitViewTypeException, this._viewType, message)) {
            this._winModel = UIType[this._viewType] as cc_zest_win_model;
        }
    }
     ...
     * 注：调用 addFilePath 函数增加某个文件时，文件路径不能出现扩展名，即如果是图片，不能出现诸如.png这种扩展名，以此类推，所有资源文件都不能出现扩展名，这是 cocos 资源加载的特性
     * @example
     *  lisAssetUrls(assetRegister: IAssetRegister) {
     *      assetRegister.addFilePath('/目录名/holleworld');
     *      assetRegister.addFilePath('/目录名/game');
     *      assetRegister.addDirPath('/资源目录');
     *  }
     */
    protected listAssetUrls(assetRegister: IAssetRegister): void { }
     ...
     */
     protected initPageSize() {
        if (!this.node.getComponent(AdapterWidget) && this._viewType !== UIType.TOAST_LAYER) {
            app.adapter.adapterWidth(this.node);
            app.adapter.adapterHeight(this.node);
        }
     }
