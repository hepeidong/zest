...
export class Loader implements ILoader {
     ...
     */
    public load<T extends Asset>(paths: string, type: cc_zest_loader_AssetType<T>, onProgress: (finish: number, total: number, item: AssetManager.RequestItem) => void, onComplete: (error: Error, assets: T) => void): void;
    public load<T extends Asset>(paths: string[], type: cc_zest_loader_AssetType<T>, onProgress: (finish: number, total: number, item: AssetManager.RequestItem) => void, onComplete: (error: Error, assets: Array<T>) => void): void;
    ...
    public load<T extends Asset>(paths: string, type: cc_zest_loader_AssetType<T>, onComplete?: (error: Error, assets: T) => void): void;
    public load<T extends Asset>(paths: string[], type: cc_zest_loader_AssetType<T>, onComplete?: (error: Error, assets: Array<T>) => void): void;
     ...
     *@param onComplete 当加载所有资源或发生错误时调用的回调。
     */
    public loadDir<T extends Asset>(dir: string, type: cc_zest_loader_AssetType<T>, onProgress: (finish: number, total: number, item: AssetManager.RequestItem) => void, onComplete: (error: Error, assets: Array<T>) => void): void;
    ...
    public loadDir<T extends Asset>(dir: string, type: cc_zest_loader_AssetType<T>, onComplete: (error: Error, assets: Array<T>) => void): void;
    public loadDir<T extends Asset>(dir: string, type: cc_zest_loader_AssetType<T>): void;
    ...
    public loadScene() {
        ...
        if (args.options) {
            if (args.onProgress) {
                this._bundle.loadScene(args.sceneName, args.options, args.onProgress, (err, assets) => {
                    this.loadComplete(err, assets, args.onComplete);
                });
            }
            else {
                this._bundle.loadScene(args.sceneName, args.options, (err, assets) => {
                    this.loadComplete(err, assets, args.onComplete);
                });
            }
        }
            ...
            if (args.onProgress) {
                this._bundle.loadScene(args.sceneName, args.onProgress, (err, assets) => {
                    this.loadComplete(err, assets, args.onComplete);
                });
            }
            else {
                this._bundle.loadScene(args.sceneName, (err, assets) => {
                    this.loadComplete(err, assets, args.onComplete);
                });
            }
        }
