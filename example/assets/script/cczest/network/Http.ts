...
export  class Http implements IHttpManager {
    ...
    private static _ins: Http = null;
    public static get instance(): Http {
        return this._ins = this._ins ? this._ins : new Http();
    }
    ...
    public get<T>(proxyName: string, _type: new () => T): cc_zest_http_message<T> {
        if (app.game.hasProxy(proxyName)) {
            return app.game.retrieveProxy(proxyName) as cc_zest_http_message<T>;
        }
        ...
        if (Assert.handle(Assert.Type.GetHttpMessageClassException, classRef, proxyName)) {
            const proxy = new classRef(proxyName) as cc_zest_http_message<T>;
