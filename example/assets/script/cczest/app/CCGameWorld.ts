...
import { Constructor, IAssetRegister, IDocument, IGameWorld, IGameWorldExtend, ISceneManager, IViewComplete, IViewProgress, ViewComplete, ViewProgress } from "zest";
import { js } from "cc";
...
export class CCGameWorld extends Facade implements IGameWorld, ISchedulable {
    ...
    constructor() {
        ...
        // this._platform   = GamePlatform.PREVIEW;
    ...
    public static getInstance() {
        return Facade.getInstance() as IGameWorldExtend;
    }
     ...
     * @returns 
     */
    public getModel<T extends IDocument>(proxyName: string, _type?: new() => T): T {
        if (this.hasProxy(proxyName)) {
            return this.retrieveProxy(proxyName) as T;
        }
     ...
     * @returns 
     */
    public removeModel(proxyName: string) {
        if (this.hasProxy(proxyName)) {
            this.removeProxy(proxyName);
            return true;
        }
        return false;
    }
    ...
    /**
     * 游戏启动完成，在首场景加载显示后调用
     * @returns 返回游戏所在的平台，请调用app.Platform进行设置
     * @example startup() { return app.Platform.PREVIEW; }
     */
    protected startup(): number{ return GamePlatform.PREVIEW; }
    /**
     * 游戏进入后台 
