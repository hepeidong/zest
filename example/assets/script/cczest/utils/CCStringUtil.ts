...
export  class CCStringUtil {
    ...
    public static logFormat(logTag: string, type: string): string {
        if (SHOW_DATE) {
            return `${logTag ? `ZEST [${logTag}]` : 'ZEST'}${CCDateUtil.dateFormat('%s-%s-%s %s:%s:%s:%s')} ${type}`;
        }
        return `${logTag ? `ZEST [${logTag}]` : 'ZEST'} ${type}`;
    }
    ...
    public static randomString(len: number): string {
        ...
        let str: string = '';
        for (let i: number = 0; i < len; i++) {
            let idx: number = CCMathUtil.randomInt(0, words.length);
